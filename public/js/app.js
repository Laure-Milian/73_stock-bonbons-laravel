...
var app = {
	...
	getData: function getData() {
		...
		axios.get("/api/candies").then(function (response) {
			return _this.displayList(response.data);
		});
	},

	displayList: function displayList(data) {
		var len = data.length;
		for (var i = 0; i < len; i++) {
			$('#list').append('<tr>' + '<td>' + data[i].name + '</td>' + '<td id="stock_' + data[i].id + '">' + data[i].stock + '</td>' + '<td> <button data-id="' + data[i].id + '" class="btnAddToStock">Ajouter</button> </td>' + '<td> <button data-id="' + data[i].id + '" class="btnSubtractToStock">Supprimer</button> </td>' + '</tr>');
		}
		this.listeners();
	},
	...
	addToStock: function addToStock() {
		var id = $(this).data("id");
		var action = "add";
		axios.put("/api/changeStock/" + action + "/" + id).then(function (response) {
			return app.changeStockValue(response.data, id);
		});
	},

	subtractToStock: function subtractToStock() {
		var id = $(this).data("id");
		var action = "subtract";
		axios.put("/api/changeStock/" + action + "/" + id).then(function (response) {
			return app.changeStockValue(response.data, id);
